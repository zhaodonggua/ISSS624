...
Based on the R-squre result and the RMSE value.doubly constrained model has the largest R-squre value and smallest RMSE value,However, the relationship between many parameters is NA,so it cannot be used. So we choose the second largest R-squre value model,the original constraint model, and the R-squre is 0.37.
The coefficient for TRAIN_COUNT is the most significant, This might be because larger areas with more of stations—like retail centers or city centers, where people move around more—are typically those with higher population densities.
Followed by the ENTERN_COUNT and FB count,This may be due to the negative correlation in restaurants and entertainment venues where people stay indoors for long periods of time.
FOr the last HDB_count,The correlation is weaker because of the increase in teleworking nowadays so people are not in the habit of going out .
